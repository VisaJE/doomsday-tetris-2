...
UberBlockifier::UberBlockifier(int maxSize)
    : generator(time(NULL)), maxSize(maxSize), nextBlock{0, 0, {}}
{
    setNewBlock();
}
...
void UberBlockifier::setNewBlock()
{
    int r = (rand() % 100) % 12;
    if (r == 4)
    {
        try
        {
            nextBlock = makeRandom();
        }
        catch (int e)
        {
            cout << "makeRandom unexpected error";
            nextBlock = makeRandom();
        }
    }
    else
...
Block UberBlockifier::getRandom() { return makeRandom(); }
    ...
    while (i < times)
    {
        try
        {
            ...
            if ((b.width == 2 && b.height == 2) &&
                ((b.isThere(0, 0) && !b.isThere(0, 1)) || (b.isThere(0, 1) && !b.isThere(0, 0))))
            {
